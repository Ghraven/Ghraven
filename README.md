...
| [pydantic/pydantic-ai](https://github.com/pydantic/pydantic-ai) ⭐17k | Added explicit `encoding="utf-8"` to both `open()` call sites in `scripts/scrub_cassette.py` — the VCR cassette redaction script now round-trips multilingual test fixtures safely on Windows / non-utf-8 locales · Read cassette-checker test sources with explicit UTF-8 encoding · Made `AgentSpec` reject YAML/JSON inputs that parse to non-object values with a clearer `UserError` |
