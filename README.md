...
<p align="left">
  <img src="https://img.shields.io/badge/PRs_opened-216-blue?style=flat-square" alt="PRs opened" />
  ...
  <img src="https://img.shields.io/badge/Contributed_to_repos_with-1004k%2B_⭐-yellow?style=flat-square" alt="Combined stars of contributed repos" />
</p>
...
| [run-llama/llama_index](https://github.com/run-llama/llama_index) ⭐50k | Set inner OpenAI client `max_retries=0` to prevent `AuthenticationError` from being silently retried · Added explicit `encoding="utf-8"` to text-mode `open()` call sites across `llama-index-core`, embedding adapter config save/load paths, and FAISS map-store ID map JSON persistence · Switched ingestion pipeline async executor path to `asyncio.get_running_loop()` · Reused the shared `asyncio_run()` helper in the readability web reader sync wrapper · Read and wrote the group-conversations docs example JSON with explicit UTF-8 encoding |
